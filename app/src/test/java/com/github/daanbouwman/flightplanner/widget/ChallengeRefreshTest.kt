...
/** The alarm time is local midnight, whatever the clock offset does that night. */
class ChallengeRefreshTest {
    ...
    fun `the next midnight is the start of the following local day`() {
        val next = ChallengeRefresh.nextLocalMidnightMillis(at("2026-09-16T14:30:00+02:00[Europe/Amsterdam]"), amsterdam)
    ...
    fun `just before midnight it is a minute away, just after it is a day away`() {
        ...
        Duration.ofMillis(ChallengeRefresh.nextLocalMidnightMillis(before, amsterdam) - before.toEpochMilli()) shouldBe
            Duration.ofMinutes(1)
        Duration.ofMillis(ChallengeRefresh.nextLocalMidnightMillis(after, amsterdam) - after.toEpochMilli()) shouldBe
    ...
    fun `across the spring clock change the day is twenty-three hours long`() {
        ...
        val midnightBefore = ChallengeRefresh.nextLocalMidnightMillis(eve, amsterdam)
        val midnightAfter = ChallengeRefresh.nextLocalMidnightMillis(Instant.ofEpochMilli(midnightBefore), amsterdam)
    ...
    fun `the zone is the widget's, not the machine's`() {
        ...
        val tokyo = ChallengeRefresh.nextLocalMidnightMillis(now, ZoneId.of("Asia/Tokyo"))
        val utc = ChallengeRefresh.nextLocalMidnightMillis(now, ZoneId.of("UTC"))
