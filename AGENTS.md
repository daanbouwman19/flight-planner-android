...
The project provides specialized skills in `.agents/skills/`:
- **`compose-expert`** (`.agents/skills/compose-expert/SKILL.md`): Advanced Jetpack Compose UI patterns, state hoisting, recomposition optimization, and shared composable structure.
- **`kotlin-specialist`** (`.agents/skills/kotlin-specialist/SKILL.md`): Idiomatic Kotlin, coroutine concurrency, Flow streams, sealed class hierarchies, and type-safe DSLs.
- **`material-3`** (`.agents/skills/material-3/SKILL.md`): Material Design 3 tokens, component specs, and adaptive layout guidance.
